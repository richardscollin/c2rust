...
pub mod properties {
    ...
    impl Mutability {
        pub fn to_static_mutability(&self, span: proc_macro2::Span) -> syn::StaticMutability {
            match self {
                Mutability::Mutable => syn::StaticMutability::Mut(Token![mut](span)),
                Mutability::Immutable => syn::StaticMutability::None,
            }
        }
    }
...
impl Builder {
    ...
    pub fn pub_(self) -> Self {
        let tok = Token![pub](self.span);
        self.vis(Visibility::Public(tok))
    }
    ...
    where
        K: Make<PathSegment>,
    {
        let path: Path = vec![key].make(&self);
        self.prepared_attr(Meta::Path(path))
    }
        ...
        V: Make<Ident>,
    {
        ...
        let arguments = arguments
            .into_iter()
            .map(|x| TokenTree::Ident(x.make(&self)))
            .collect();
        ...
        self.prepared_attr(Meta::List(metalist))
    }
        ...
        S: Make<PathSegment>,
    {
        ...
        let turbofish = match seg.arguments {
            ...
            PathArguments::AngleBracketed(ab) => Some(AngleBracketedGenericArguments {
    ...
    pub fn foreign_items(self, items: Vec<ForeignItem>) -> Box<Item> {
        ...
        // TODO may want to restrict emitting unsafe here to only >= 2024 edition
        let unsafety = match self.unsafety {
            Unsafety::Normal => None,
            Unsafety::Unsafe => Some(Token![unsafe](self.span)),
        };
    ...
    pub fn self_arg(self, kind: SelfKind, ty: Box<Type>) -> FnArg {
    ...
    pub fn attribute<Ma>(self, style: AttrStyle, args: Ma) -> Attribute
    where
        Ma: Make<Meta>,
    {
        let args = args.make(&self);
    ...
    pub fn meta_item_attr(mut self, style: AttrStyle, meta: Meta) -> Self {
        let attr = self.clone().attribute(style, meta);
